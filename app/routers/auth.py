...
def login(user_credentials: OAuth2PasswordRequestForm=Depends(), db: Session=Depends(database.get_db)):
    ...
    user=db.query(models.User).filter(models.User.username==user_credentials.username).first() #username not email because of the OAuth form
    ...
    return {"access_token": access_token, "token_type":"bearer"} #returning access token